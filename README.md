...
1. Загрузите тестовую базу данных  
   - Используйте выгрузку [diplom.dt](diplom.dt)  
   - Восстановите через Конфигуратор: Администрирование → Загрузить информационную базу  

2. Подготовьте файлы тестирования  
   - Сохраните в один каталог:  
     - Все сценарии автоматизированного тестирования  
     - Эталон отчета "Анализ выставленных актов"  

3. Установите Vanessa Automation  
   - Скачайте single-версию из [официального репозитория](https://github.com/Pr-Mex/vanessa-automation)  
   - Разместите `vanessa-automation-single.epf` в удобном каталоге  

4. Настройте базу для тестирования  
   - В режиме Конфигуратора:  
     - Откройте: Администрирование → Пользователи  
     - Выберите пользователя "Администратор"  
     - Снимите галочку с поля "Защита от опасных действий" (Рисунок 1)  

5. Задайте параметры запуска  
   - В ярлыке запуска 1С укажите:  
     ```
     /TestManager /N Администратор /P "" /Execute "<полный_путь>/vanessa-automation-single.epf"
     ```  
      

6. Запустите тестовую базу  
   - Откройте базу в режиме 1С:Предприятие  
   - Во всплывающем окне подтвердите использование внешней компоненты "VanessaExt"   

7. Настройте сценарии тестирования  
   - В открывшемся интерфейсе Vanessa Automation:  
     - Перейдите на вкладку "Добро пожаловать!"  
     - Нажмите "Открыть папку"  
     - Выберите каталог с подготовленными сценариями   

8. Запустите выполнение тестов  
   - Нажмите кнопку "Выполнить"  
   - Дождитесь завершения процесса  

9. Проверьте результаты  
   - При успешном выполнении: появится сообщение об успехе  
   - При ошибках: обратитесь к разработчику с логами тестирования 
   </details>
